...
if useadvBDTdat:
    ...
    # "p_etcone30ptCorrection","p_etcone40ptCorrection","p_ptcone20","p_ptcone30","p_ptcone40",
    # "p_numberOfInnermostPixelHits","p_numberOfPixelHits","p_numberOfSCTHits","p_d0","p_d0Sig","p_dPOverP",
    # "p_deltaEta1","p_deltaPhiRescaled2","p_EptRatio","p_TRTPID","p_numberOfTRTHits","p_TRTTrackOccupancy",
    # "p_numberOfTRTXenonHits","Z_m","p_LHValue","p_et_calo","mva_Calo_kBDT_conf1_mc","mva_Iso_kBDT_conf1_mc",
    # "mva_Track_kBDT_conf1_mc","mva_kBDT_conf1_mc_final","label0","label1","p_TruthType","Truth"]
    ...
    testX = valdata[:, range(0, 7 + 1) + range(20, 29 + 1)]  # 0-7 er calorimeter, 20-29 er tracking
    testy = []
    for i, x in enumerate(valdata):
        if valdata[i, [40]] == 1 and valdata[i, [42]] == 2:  # label0 and p_TruthType
            testy.extend([1])
        else:
            testy.extend([0])
